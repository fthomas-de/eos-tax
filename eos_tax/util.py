...
def corp_has_payed(corp_id:int, month:int, year:int):
    ...
    if USE_REASON:
        payments = CorporationWalletJournalEntry.objects.filter(second_party_id__in=TAX_CORPORATIONS, ref_type__in=DONATION_TYPES, reason=f"{corp_id}/{month}/{year}", amount__gte=amount_to_pay*-1).values('id').all()
        if len(payments) == 0:
            payments = CorporationWalletJournalEntry.objects.filter(second_party_id__in=TAX_CORPORATIONS, ref_type__in=DONATION_TYPES, reason=f"{corp_id}/{month}/{year}", amount__gte=amount_to_pay).values('id').all()
    else:
